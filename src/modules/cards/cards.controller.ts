...
export const getCardById = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    const card = await cardsService.getCardById(req.userId!, req.params['id'] as string)
    res.json(card)
  } catch (err) {
    next(err)
  }
...
export const updateCard = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    const card = await cardsService.updateCard(req.userId!, req.params['id'] as string, req.body)
    res.json(card)
  } catch (err) {
    next(err)
  }
...
export const deleteCard = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    await cardsService.deleteCard(req.userId!, req.params['id'] as string)
    res.status(204).send()
  } catch (err) {
    next(err)
  }
